...
# to avoid false positives like 'r' matching 'recruitment'
...
def extract_skills(text, job_skills=None):
    ...
    # 1. First match job-specific skills (IMPORTANT)
    if job_skills:
        for skill in job_skills:
            if skill.lower() in text_lower:
                found_skills.add(skill.lower())

    # 2. Then match from global database (backup)
    for skill in SKILLS_DATABASE:
        if skill.lower() in text_lower:
            found_skills.add(skill.lower())
...
def extract_experience(text):
    ...
    # 1. Direct patterns like "3 years"
    patterns = [
    ...
    # 2. Detect internship / fresher
    if 'intern' in text_lower or 'freelance' in text_lower:
        return "0-1 years"

    # 3. Detect date ranges (but ignore year numbers)
